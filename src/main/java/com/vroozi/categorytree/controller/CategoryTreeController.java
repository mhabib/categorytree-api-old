...
public class CategoryTreeController {
	...
	@RequestMapping(value = "/category-tree/cvgroup/company/{unitid}/groupid/{groupid}/name/{name}/cgrouptoken/{cGroupToken}/active/{active}", method = RequestMethod.POST)
	public void addUpdateContentViewGroup(@PathVariable("groupid")String groupId, @PathVariable("unitid")String unitId, @PathVariable("name")String name, @PathVariable("cGroupToken")String token, @PathVariable("active")Boolean active) {
		
		categoryTreeService.addUpdateContentViewGroup(groupId, unitId, name, token, active);
	}

	@RequestMapping(value = "/category-tree/cvgroup/goupids/{goupids}/active/{active}", method = RequestMethod.PUT)
	public void updateProfileGroups(@PathVariable("goupids") String groupIds, @PathVariable("active") Boolean active) {
	...
	@RequestMapping(value = "/category-tree/cview/cviewid/{cviewid}/cvname/{cvname}/active/{active}", method = RequestMethod.POST)
	void addUpdateContentView(@PathVariable("cviewid") String contentViewId, @PathVariable("cvname") String cvname, @PathVariable("active") Boolean active) {
		categoryTreeService.addUpdateContentView(contentViewId, cvname, active);
	}
	
	@RequestMapping(value = "/category-tree/cview/cviewids/{cviewids}/active/{active}", method = RequestMethod.PUT)
	void updateContentView(@PathVariable("cviewids") String contentViewIds, @PathVariable("active") Boolean active) {
...
//	}
	...
	@RequestMapping(value = "/category-tree/catalog/company/{unitid}/catalogid/{catalogid}", method = RequestMethod.DELETE)
	...
	@RequestMapping(value = "/category-tree/upload/company/{unitid}/basefile/{basefile}", method = RequestMethod.POST)
	public void uploadCategories(@PathVariable("unitid")String unitId, @PathVariable("basefile")String baseFile) {
	...
	public @ResponseBody
	Category getSearchResultsBasedOnUnitIdAndCGroupToken(
		...
		Category categoryMappings = categoryTreeService.getCategoryTree(unitId,
				cGroupToken);
